...
Chain-Native Pattern is an dApp design pattern based on **“Native Extension”**. In **Native Extension**, extension program can be added dynamically to the running blockchain. By doing so, it is possible to modify the default behaviors or add new behaviors when processing transactions.
