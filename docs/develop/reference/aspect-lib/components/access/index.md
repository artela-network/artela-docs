...
This chapter introduces how to use Context Key to obtain context data, as well as the access rights of each key at each cut point.
...
* **Example:**
  * ```javascript
    const key="isCall";
    const isCall = sys.hostApi.runtimeContext.get(key);
    if (isCall) {
