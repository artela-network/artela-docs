...
To dive deeper, visit the [Intro to Artela](main/intro-to-artela).
...
Artela introduces an innovative programming model known as **Aspect**, which enables developers to add functionalities and logic layers without modifying the original contract code. Aspect allows developers to dynamically adjust contract behavior during execution by injecting "joinpoints," making it easier to implement complex business logic and security features. For developers looking to build rich, feature-packed dApps, Aspect is a powerful tool.
...
For detailed usage, please refer to the [Developers Guide](main/developers-guid).